...
이러한 과목을 만든 사람들은 우리보다 잘해서 그만큼 높은 효율
...
사람은 기본적으로 문제를 만났을때 경험 기반 사고를 한다.
...
- 많은 라이브러리
- MVC 비동기
- 소프트 웨어 퀄리티 : ooad, tdd, 애자일
- 백엔드에서 전향한 사람
- 다른 언어 프레임워크 경험자
  ==> 결론은 이런 것을 하는 사람들이 컴퓨터 사이언티스트에 큰접하지 않을까?
...
- 하다보면 여기서는 쉽게 되는데, 여기서는 왜 이렇게 안되는 것일까? ????? 플랫폼이라고 하 면 웹기반? 아니면 아예 다른 언어 기반?
...
var isSale = function(id) {
    ...
    if( product !== null ) {
        if () {
            if()......
            if( isInRange(product.saleStateDate, product.salseEndDate) )...  
            //자기 설명적인 코드
        }
    }
}
...
남자끼리 엮었을때 줄을 선 아이돌 그룹 멤버들과 팬들의 성별이 임의로 주어질때, 팬 사인회가 진행되는 과정에서 아이돌 그룹의 모든 멤버가 동시에 포옹을 하는 일이 몇번 일어나는지를 구하는 프로그램을 작성하세요.
